...
import type WebSocketConnection from "../Services/WebSocketConnection.js";
import type TextMessage from "../WebSocketMessages/TextMessage.js";
import type { AnsiLogger } from "node-ansi-logger";
import type { LoxoneClientOptions } from "../LoxoneClientOptions.js";
...
class TokenHandler {
  ...
  async refreshToken(): Promise<void> {
    ...
    if (msUntilExpiry < 0) {
      this.log.warn(
        "Token cannot be refreshed any more as it expired. Trying to acquire a new one",
      );
    ...
    const refreshTokenResponse =
      await this.connection.sendEncryptedTextCommand(refreshTokenCommand);
  ...
  async authenticateWithToken(token: string): Promise<void> {
    ...
    const authWithTokenResponse =
      await this.connection.sendEncryptedTextCommand(authWithTokenCommand);
  ...
  private processTokenResponse(tokenResponse: TextMessage): void {
    ...
    const seconds = Number.parseInt(this.validUntil);
  ...
  private scheduleRefresh(): void {
    ...
    this.refreshTimer = setTimeout(() => {
      void this.runScheduledRefresh(msUntilExpiry);
    }, finalMs);
  }

  // Executes the scheduled refresh, retrying with backoff on failure.
  private async runScheduledRefresh(msUntilExpiry: number): Promise<void> {
    try {
      ...
      if (this.refreshRetries <= this.refreshMaxRetries) {
        this.refreshTimer = setTimeout(
          () => this.scheduleRefresh(),
          this.refreshRetryMs * this.refreshRetries,
        );
      } else {
