...
import type { LoxoneEvent, LoxoneEventCtor } from "../LoxoneEvents/LoxoneEvent.js";
...
import { type AnsiLogger, GREY } from "node-ansi-logger";
import { maskEnc } from "../Utils/Masker.js";
import { rawDataToString } from "../Utils/RawData.js";
...
class WebSocketConnection extends EventEmitter {
  ...
  async connect(): Promise<void> {
    ...
    this.ws.on("close", (code: number, reason: Buffer) => {
      this.cleanupAfterDisconnectOrError(
        `Closed with code ${code}, reason: ${reason.toString("utf8")}`,
      );
    });
  ...
  cleanupAfterDisconnectOrError(reason: string): void {
  ...
  handleMessage(data: WebSocket.RawData, isBinary: boolean): void {
    switch (this.nextExpectedMessageType) {
      ...
      case MessageType.TEXT: {
        ...
        const textMessage = new TextMessage(rawDataToString(data));
      ...
      default:
        this.log.error(`Received unexpected message type ${this.nextExpectedMessageType}`);
        break;
    }
  }

  private parseEventTables<T extends LoxoneEvent>(
    ctorType: LoxoneEventCtor<T>,
  ...
  async sendEncryptedTextCommand(
    command: string,
  ...
  async sendUnencryptedTextCommand(
    command: string,
    timeoutMs: number = this.COMMAND_TIMEOUT,
  ): Promise<TextMessage> {
  ...
  async sendUnencryptedFileCommand(
    filename: string,
    timeoutMs: number = this.COMMAND_TIMEOUT,
  ): Promise<FileMessage> {
  ...
  async sendCommand<T extends FileMessage | TextMessage>(
    command: string,
    encrypt = false,
    timeoutMs: number = this.COMMAND_TIMEOUT,
  ): Promise<T> {
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      const commandDefinition = {
        ...
        encryptedCommand: encrypt
          ? this.loxoneClient.auth.commandEncryption.getEncryptedCommand(command)
          : undefined,
      };
      ...
      try {
        if (commandDefinition.encryptedCommand) {
          if (this.messageLog)
            this.log.info(
              `Sending encrypted command ${command} ${GREY}(${maskEnc(commandDefinition.encryptedCommand)})`,
            );
  ...
  private findCommandQueueEntryIndex(command: string): number {
    const i = this.commandQueue.findIndex(
      (q) =>
        (q.command.encryptedCommand &&
          // oxlint-disable-next-line typescript/prefer-nullish-coalescing
          decodeURIComponent(q.command.encryptedCommand) === command) ||
        (q.command.encryptedCommand &&
          // oxlint-disable-next-line typescript/prefer-nullish-coalescing
          decodeURIComponent(q.command.encryptedCommand).replace("jdev", "dev") === command) ||
  ...
  override on<K extends keyof WebSocketConnectionEvents>(
    event: K,
    listener: WebSocketConnectionEvents[K],
  ): this {
    // oxlint-disable-next-line typescript/no-explicit-any
    return super.on(event, listener as (...args: any[]) => void);
  }

  override once<K extends keyof WebSocketConnectionEvents>(
    event: K,
    listener: WebSocketConnectionEvents[K],
  ): this {
    // oxlint-disable-next-line typescript/no-explicit-any
  ...
  override off<K extends keyof WebSocketConnectionEvents>(
    event: K,
    listener: WebSocketConnectionEvents[K],
  ): this {
    // oxlint-disable-next-line typescript/no-explicit-any
  ...
  override emit<K extends keyof WebSocketConnectionEvents>(
    event: K,
    ...args: Parameters<WebSocketConnectionEvents[K]>
  ): boolean {
