...
class ParsedHeader extends WsBinHdr {
  ...
  constructor(cBinType = 0x03, cIdentifier = 0, cInfo = 0, cReserved = 0, nLen = 0) {
    ...
    // parse message type
    const candidate = this.cIdentifier;
   ...
   * @param {Buffer} buf - Buffer containing the raw header bytes.
   * @param {number} offset - Byte offset within the buffer to start reading at.
   * @returns {ParsedHeader} The parsed header.
   */
   ...
   * @param {WebSocket.RawData} raw - Raw data received from the WebSocket.
   * @param {boolean} isBinary - Whether the received message was binary.
   * @returns {ParsedHeader} The parsed header.
   */
