...
class LoxoneTextEvent extends LoxoneEnrichableEvent {
  ...
  override data_length(): number {
    return (
      (Math.floor(
        (4 + this.textLength + this.uuid.data_length + this.uuidIcon.data_length - 1) / 4,
      ) +
        1) *
      4
    );
  ...
  override toPath(): string {
    ...
    const controlString = parentControl
      ? `${parentControl.name}${GREY}/${YELLOW}${parentControl.name}`
      : "unknown";
    const roomString = parentControl?.room.name ?? "unknown";
    return this.isEnriched
      ? `${YELLOW}${roomString}${GREY}/${YELLOW}${controlString}${GREY}/${YELLOW}${this.state?.name}${GREY}`
      : this.uuid.stringValue;
  }
